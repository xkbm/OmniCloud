...
export async function syncStorageAccount(env, userId, account) {
  ...
  if (summary) {
    await db`
      ...
      SET total_space = ${Number(summary.totalSpace || account.total_space || 0)}, used_space = ${Number(summary.usedSpace || 0)}, updated_at = NOW()
  ...
  for (const record of records) {
    await db`
      INSERT INTO file_metadata (
        ...
        ${crypto.randomUUID()}, ${userId}, ${record.virtual_path || '/'}, ${record.file_name}, ${Boolean(record.is_folder)}, ${Boolean(record.is_starred)},
        ${Number(record.size || 0)}, ${record.mime_type || null}, ${account.id},
        ${String(record.remote_file_id || `${account.provider}:${record.virtual_path || '/'}:${record.file_name}`)},
        ${record.remote_parent_id || null}, ${record.remote_created_time || null}, ${record.remote_modified_time || null}
      )
      ON CONFLICT (cloud_account_id, remote_file_id) DO UPDATE SET
        virtual_path = EXCLUDED.virtual_path, file_name = EXCLUDED.file_name, is_folder = EXCLUDED.is_folder,
        size = EXCLUDED.size, mime_type = EXCLUDED.mime_type, remote_parent_id = EXCLUDED.remote_parent_id,
        remote_created_time = EXCLUDED.remote_created_time, remote_modified_time = EXCLUDED.remote_modified_time, updated_at = NOW()
    `;
  }
...
export async function performUpload(env, account, input) {
  ...
  return adapter.uploadStream({
    stream: nodeReadableFromWeb(input.body), size: input.size, fileName: input.fileName, mimeType: input.mimeType,
    virtualPath: input.virtualPath, remoteParentId: input.remoteParentId, onProgress: input.onProgress,
  });
}
