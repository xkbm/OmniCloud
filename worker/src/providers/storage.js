...
export async function syncStorageAccount(env, userId, account) {
  if (account.provider === 'google_drive') {
    return syncGoogleAccount(env, userId, account);
  }
  ...
  if (summary) {
    await db`
      ...
      SET total_space = ${Number(summary.totalSpace || account.total_space || 0)},
          used_space = ${Number(summary.usedSpace || 0)},
          updated_at = NOW()
  ...
  for (const record of records) {
    await db`
      INSERT INTO file_metadata (
        ...
        ${crypto.randomUUID()}, ${userId}, ${record.virtual_path || '/'}, ${record.file_name},
        ${Boolean(record.is_folder)}, ${Boolean(record.is_starred)}, ${Number(record.size || 0)}, ${record.mime_type || null},
        ${account.id}, ${String(record.remote_file_id || `${account.provider}:${record.virtual_path || '/'}:${record.file_name}`)},
...
export async function performDownload(env, account, row) {
  ...
  const { adapter } = await getStorageAdapter(env, account);
  return new Response(adapter.getDownloadStream(row));
}
...
export async function performUpload(env, account, input) {
  ...
  const { adapter } = await getStorageAdapter(env, account);
  const stream = nodeReadableFromWeb(input.body);
...
export async function setStar(env, account, row, isStarred) {
  if (account.provider !== 'google_drive') {
    throw Object.assign(new Error(`Starred state is not supported for provider ${account.provider}`), { status: 409 });
  }
