...
async function reconcileUpload(db, saga) {
  ...
  const rows = await db`
    ...
    WHERE id=${uploadId} AND user_id=${saga.user_id}
    LIMIT 1
  `;
  ...
  await db`
    ...
    VALUES
      (${crypto.randomUUID()},${saga.user_id},${session.virtual_path},${session.file_name},FALSE,FALSE,${Number(session.size || 0)},${session.mime_type || null},${session.cloud_account_id},${String(remoteId)},${session.remote_parent_id || null})
...
async function reconcileRename(db, saga) {
  const row = saga.file_id
    ? (await db`SELECT id FROM file_metadata WHERE id=${saga.file_id} AND user_id=${saga.user_id} LIMIT 1`)[0]
    : null;
  if (!row) return;
  ...
  await db`
    ...
    SET file_name=${newName}, updated_at=NOW()
    WHERE id=${saga.file_id} AND user_id=${saga.user_id}
  `;
}
...
async function reconcileMove(env, db, saga) {
  ...
  if (row.is_folder && oldFolderPrefix) {
    ...
    await db`
      ...
      SET virtual_path = CASE
        WHEN id=${row.id} THEN ${newFolderPrefix}
        ELSE ${newFolderPrefix} || substring(virtual_path from ${oldFolderPrefix.length + 1})
      END,
  ...
  await db`
    UPDATE file_metadata
    SET virtual_path=${destinationPath}, remote_parent_id=${destinationParentId}, updated_at=NOW()
    WHERE id=${row.id} AND user_id=${saga.user_id}
  `;
}
    ...
    try {
      switch (saga.operation) {
        ...
        case 'delete':
          ...
          break;
        default:
          throw new Error(`Unsupported saga operation: ${saga.operation}`);
      }
