...
async function consumeOAuthState(env, state, provider) {
  ...
  const rows = await db`
    ...
    LIMIT 1
  `;
  if (!rows[0]) throw new Error(`Invalid or expired ${provider} OAuth state`);
...
function normalizeMegaConnectError(error) {
  if (isMegaInvalidCredentialError(error)) return new Error('MEGA email or password is incorrect.');
  return error;
}
    ...
    try {
      ...
      try {
        ...
        const account = await upsertAccount(env, {
          ...
          email: accountEmail,
          provider: 'mega',
          credentials: { provider: 'mega', email: accountEmail, password, secondFactorCode: secondFactorCode || null, session },
...
async function connectS3(env, userId, body) {
  ...
  try {
    ...
    if (status === 404 || name === 'NotFound' || name === 'NoSuchBucket') throw new Error(`Bucket "${bucket}" was not found on this endpoint.`);
  ...
  const account = await upsertAccount(env, {
    userId, email, provider: 's3',
    credentials: { provider: 's3', accessKeyId, secretAccessKey, bucket, region, endpoint, forcePathStyle: forcePathStyle !== false },
    totalSpace: Number(totalSpace) || DEFAULT_S3_TOTAL_SPACE,
    usedSpace: 0,
  });
...
async function connectPCloud(env, userId, body) {
  ...
  const account = await upsertAccount(env, {
    userId, email: login.email || username, provider: 'pcloud',
    credentials: { provider: 'pcloud', username, password, host: login.host, auth: login.auth },
    totalSpace: login.totalSpace, usedSpace: login.usedSpace,
  });
...
export async function accountsRoutes(app) {
  ...
  for (const provider of ['google', 'onedrive', 'dropbox', 'yandex', 'mega', 's3', 'pcloud']) {
    app.get(`/api/accounts/${provider}/status`, async (c) => {
      ...
      catch (error) { return c.json({ error: error?.message || 'Request failed' }, error instanceof Response ? error.status : 400); }
    });
  }

  app.get('/api/accounts/onedrive/connect', async (c) => {
    try {
      const user = await requireUser(c); const config = oauthConfig(c.env, 'onedrive'); const state = await saveOAuthState(c.env, user.id, 'onedrive');
      const url = new URL(`${config.authority}/authorize`);
      url.searchParams.set('client_id', config.clientId); url.searchParams.set('response_type', 'code'); url.searchParams.set('redirect_uri', config.redirectUri); url.searchParams.set('response_mode', 'query'); url.searchParams.set('scope', ONEDRIVE_SCOPE); url.searchParams.set('state', state);
      return c.json({ data: { authorizationUrl: url.toString(), state, redirectUri: config.redirectUri } });
    } catch (error) { return c.json({ error: error?.message || 'Unable to start OneDrive OAuth' }, 400); }
  });
  ...
  app.get('/api/accounts/onedrive/callback', async (c) => {
    ...
    try {
      ...
      const userId = await consumeOAuthState(c.env, c.req.query('state') || '', 'onedrive');
      const config = oauthConfig(c.env, 'onedrive'); const tokens = await exchangeCode(config, c.req.query('code') || '', { scope: ONEDRIVE_SCOPE }); const profile = await refreshOneDriveProfile(tokens.access_token);
    ...
    } catch (error) { frontend.searchParams.set('onedrive', 'error'); frontend.searchParams.set('message', error?.message || 'OneDrive OAuth failed'); }
    return c.redirect(frontend.toString());
  });

  app.get('/api/accounts/dropbox/connect', async (c) => {
    try {
      const user = await requireUser(c); const config = oauthConfig(c.env, 'dropbox'); const state = await saveOAuthState(c.env, user.id, 'dropbox'); const url = new URL('https://www.dropbox.com/oauth2/authorize');
      url.searchParams.set('client_id', config.clientId); url.searchParams.set('response_type', 'code'); url.searchParams.set('redirect_uri', config.redirectUri); url.searchParams.set('token_access_type', 'offline'); url.searchParams.set('scope', DROPBOX_SCOPES.join(' ')); url.searchParams.set('state', state);
      return c.json({ data: { authorizationUrl: url.toString(), state, redirectUri: config.redirectUri } });
    } catch (error) { return c.json({ error: error?.message || 'Unable to start Dropbox OAuth' }, 400); }
  });
  ...
  app.get('/api/accounts/dropbox/callback', async (c) => {
    ...
    } catch (error) { frontend.searchParams.set('dropbox', 'error'); frontend.searchParams.set('message', error?.message || 'Dropbox OAuth failed'); }
    return c.redirect(frontend.toString());
  });

  app.get('/api/accounts/yandex/connect', async (c) => {
    try {
      const user = await requireUser(c); const config = oauthConfig(c.env, 'yandex'); const state = await saveOAuthState(c.env, user.id, 'yandex'); const url = new URL('https://oauth.yandex.com/authorize');
      url.searchParams.set('response_type', 'code'); url.searchParams.set('client_id', config.clientId); url.searchParams.set('redirect_uri', config.redirectUri); url.searchParams.set('scope', YANDEX_SCOPE); url.searchParams.set('state', state);
      return c.json({ data: { authorizationUrl: url.toString(), state, redirectUri: config.redirectUri } });
    } catch (error) { return c.json({ error: error?.message || 'Unable to start Yandex OAuth' }, 400); }
  });
  ...
  app.get('/api/accounts/yandex/callback', async (c) => {
    ...
    } catch (error) { frontend.searchParams.set('yandex', 'error'); frontend.searchParams.set('message', error?.message || 'Yandex OAuth failed'); }
    return c.redirect(frontend.toString());
  });
  ...
  app.post('/api/accounts/mega/connect', async (c) => {
    ...
    catch (error) { return c.json({ error: error?.message || 'MEGA connection failed' }, 400); }
  });
  ...
  app.post('/api/accounts/s3/connect', async (c) => {
    ...
    catch (error) { return c.json({ error: error?.message || 'S3 connection failed' }, 400); }
  });
  ...
  app.post('/api/accounts/pcloud/connect', async (c) => {
    ...
    catch (error) { return c.json({ error: error?.message || 'pCloud connection failed' }, 400); }
  });
  ...
  app.delete('/api/accounts/:id', async (c) => {
    try {
      ...
      if (!rows[0]) return c.json({ error: 'Account not found' }, 404);
