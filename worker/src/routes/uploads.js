...
export async function uploadsRoutes(app) {
  app.post('/api/uploads/initiate', async (c) => {
    try {
      ...
      const fileName = String(body.fileName || body.file_name || '').trim(); const size = Number(body.size || 0);
      const mimeType = String(body.mimeType || body.mime_type || 'application/octet-stream'); const virtualPath = String(body.virtualPath || body.virtual_path || '/');
      if (!fileName) return c.json({ error: 'File name is required' }, 400);
      const db = sql(c.env); const requested = body.cloud_account_id || body.cloudAccountId || null;
      const accounts = requested ? await db`SELECT * FROM cloud_accounts WHERE id = ${requested} AND user_id = ${user.id} AND status = 'active' LIMIT 1` : await db`SELECT * FROM cloud_accounts WHERE user_id = ${user.id} AND status = 'active' ORDER BY used_space ASC LIMIT 1`;
      ...
      await db`INSERT INTO upload_sessions (id,token,user_id,cloud_account_id,file_name,mime_type,size,virtual_path,status) VALUES (${id},${token},${user.id},${accounts[0].id},${fileName},${mimeType},${size},${virtualPath},'pending')`;
      return c.json({ data: { id, token, provider: accounts[0].provider, cloudAccountId: accounts[0].id, status: 'pending' } });
  ...
  app.post('/api/uploads/:uploadId/stream', async (c) => {
    ...
    try {
      ...
      const rows = await db`
        ...
        FROM upload_sessions us JOIN cloud_accounts ca ON ca.id = us.cloud_account_id
        WHERE us.id = ${uploadId} AND us.user_id = ${user.id} LIMIT 1`;
      const session = rows[0]; if (!session) return c.json({ error: 'Upload session not found' }, 404);
      ...
      const total = Number(session.size || c.req.header('Content-Length') || 0); let uploaded = 0; let lastReported = 0;
        ...
        async start(controller) {
          try {
            while (true) {
              ...
              uploaded += value.byteLength; controller.enqueue(value);
      ...
      const result = await performUpload(c.env, { id: session.cloud_account_id, user_id: user.id, email: session.email, provider: session.provider, encrypted_credentials: session.encrypted_credentials, status: session.account_status, total_space: session.total_space, used_space: session.used_space }, {
        body, fileName: session.file_name, mimeType: session.mime_type, virtualPath: session.virtual_path, size: session.size,
      ...
      if (remoteId) {
        await db`
          ...
          VALUES (${crypto.randomUUID()},${user.id},${session.virtual_path},${result.fileName || session.file_name},FALSE,FALSE,${Number(result.size || session.size || 0)},${result.mimeType || session.mime_type},${session.cloud_account_id},${String(remoteId)},${result.remoteParentId || result.parentId || null},${result.createdTime || null},${result.modifiedTime || null})
          ON CONFLICT (cloud_account_id,remote_file_id) DO UPDATE SET file_name=EXCLUDED.file_name,size=EXCLUDED.size,mime_type=EXCLUDED.mime_type,updated_at=NOW()`;
      }
