...
async function sendProgress(c, uploadId, uploaded, total) {
  ...
  if (!stub) return;
  c.executionCtx.waitUntil(stub.fetch('https://upload-progress/progress', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ uploadId, uploaded, total }) }).catch(() => {}));
}
...
export async function uploadsRoutes(app) {
  ...
  app.post('/api/uploads/:uploadId/stream', async(c)=>{
    ...
    try{
      ...
      const total=Number(session.size||c.req.header('Content-Length')||0);let uploaded=0;let lastReported=0;const reader=c.req.raw.body.getReader();
      const body=new ReadableStream({async start(controller){try{while(true){const{done,value}=await reader.read();if(done)break;uploaded+=value.byteLength;if(uploaded>Number(session.size)){controller.error(new Error('Uploaded content exceeds declared file size'));return;}controller.enqueue(value);if(uploaded-lastReported>=1024*1024||(total&&uploaded>=total)){lastReported=uploaded;await sendProgress(c,uploadId,uploaded,total);}}if(uploaded!==Number(session.size)){controller.error(new Error('Uploaded content size does not match declared file size'));return;}controller.close();}catch(error){controller.error(error);}finally{reader.releaseLock();}}});
      const result=await performUpload(c.env,{id:session.cloud_account_id,user_id:user.id,email:session.email,provider:session.provider,encrypted_credentials:session.encrypted_credentials,status:session.account_status,total_space:session.total_space,used_space:session.used_space},{body,fileName:session.file_name,mimeType:session.mime_type,virtualPath:session.virtual_path,remoteParentId:session.remote_parent_id,size:session.size,onProgress:(bytes)=>c.executionCtx.waitUntil(sendProgress(c,uploadId,bytes,total))});
      await db`UPDATE upload_sessions SET status='completed',updated_at=NOW() WHERE id=${uploadId}`;const remoteId=result.remoteFileId||result.id;
      ...
      await sendProgress(c,uploadId,total||uploaded,total||uploaded);return c.json({data:{success:true,uploadId,file:result}},201);
    }catch(error){try{await sql(c.env)`UPDATE upload_sessions SET status='failed',updated_at=NOW() WHERE id=${uploadId}`;}catch{}return c.json({error:error?.message||'Upload failed'},error?.status||400);}
  });
}
