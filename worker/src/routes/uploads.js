    ...
    try {
      ...
      await db`
        INSERT INTO upload_sessions
          (id, token, user_id, cloud_account_id, file_name, mime_type, size, virtual_path, remote_parent_id, status)
        VALUES
          (${id}, ${token}, ${user.id}, ${accounts[0].id}, ${fileName}, ${mimeType}, ${size}, ${virtualPath}, ${remoteParentId}, 'pending')
      `;
      ...
      return c.json({ error: error?.message || 'Unable to initiate upload' }, error?.status || 400);
    }
  });
  ...
  app.post('/api/uploads/:uploadId/stream', async (c) => {
    ...
    try {
      ...
      const result = await performUpload(c.env, {
        ...
        size: session.size,
        onProgress: (bytes) => {
          c.executionCtx.waitUntil(sendProgress(c, uploadId, bytes, total));
        },
      });
