...
<template>
	<DriveShell current-section="home">
		<div class="min-h-[calc(100vh-84px)] rounded-[24px] bg-white px-4 py-6 text-[#202124] dark:bg-[#12161d] dark:text-slate-100 sm:px-6">
			<section class="mb-7 grid gap-5 rounded-[20px] bg-gradient-to-b from-[#e8f0fe] to-[#f1f6ff] p-7 dark:from-slate-900 dark:to-slate-800 sm:grid-cols-[minmax(0,1.6fr)_280px]">
				<div>
					...
					<div class="mt-[18px] flex flex-wrap gap-3">
						...
						<RouterLink to="/quota" class="inline-flex h-10 items-center rounded-full border border-[#dadce0] bg-white px-[18px] text-[#1a73e8] dark:border-slate-600 dark:bg-[#12161d] dark:text-sky-400">
							{{ t('nav.storage') }}
						</RouterLink>
					</div>
				</div>

				<div class="flex flex-col items-center justify-center gap-3.5 rounded-[20px] border border-[#e0e3e7] bg-white p-5 text-center dark:border-[#272e39] dark:bg-[#141821]/80">
					<div class="grid size-[116px] place-items-center rounded-full" :style="{ background: `conic-gradient(#1a73e8 0 ${storagePercentRounded}%, #eaf1fb ${storagePercentRounded}% 100%)` }">
						<div class="grid size-[82px] place-items-center rounded-full bg-white font-bold text-[#1a73e8] dark:bg-[#07090d]">{{ storagePercentRounded }}%</div>
					</div>
					<div>
			...
			<section class="mt-[26px]">
				<div class="mb-3 flex items-center justify-between gap-3">
					...
					<RouterLink to="/recent" class="rounded-full border border-[#dadce0] bg-white px-3.5 py-2 text-[#1a73e8] dark:border-slate-600 dark:bg-[#12161d] dark:text-sky-400">{{ t('home.viewAll') }}</RouterLink>
				</div>

				<div class="overflow-hidden rounded-2xl border border-[#e0e3e7] dark:border-[#272e39]">
					<div class="grid min-h-11 grid-cols-[minmax(220px,2fr)_1fr_140px] items-center gap-3 bg-[#f8fafd] px-[18px] text-[13px] text-[#5f6368] dark:bg-[#07090d]/70 dark:text-slate-400 max-md:grid-cols-[minmax(180px,1.8fr)_1fr]">
						<span>{{ t('home.fileName') }}</span>
						<span>{{ t('home.fileModified') }}</span>
						<span class="max-md:hidden">{{ t('home.fileSize') }}</span>
					</div>

					<div v-for="file in quickFiles" :key="file.id" class="grid min-h-[52px] grid-cols-[minmax(0,2fr)_minmax(0,1fr)_140px] items-center gap-3 border-t border-[#eceff1] px-[18px] dark:border-[#272e39] max-md:grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)]">
