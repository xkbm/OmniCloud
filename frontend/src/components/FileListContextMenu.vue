...
<template>
	<div v-if="contextMenu.visible" ref="contextMenuRef" class="fixed z-50 min-w-[220px] overflow-hidden rounded-2xl border border-[#e0e3e7] bg-white py-2 shadow-[0_16px_40px_rgba(32,33,36,0.2)] dark:border-[#272e39] dark:bg-[#12161d] dark:shadow-[0_16px_40px_rgba(15,23,42,0.45)]" :style="{ left: `${contextMenu.x}px`, top: `${contextMenu.y}px` }" @click.stop @contextmenu.stop>
		<button v-if="showOpen" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-[#20262f]/70" @click="handleOpen">
			<IconFolder :size="17" :stroke="2" />
			<span>{{ t('common.open') }}</span>
		</button>
		<button v-if="showPreview" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-[#20262f]/70" :disabled="!canPreview" @click="handlePreview">
			<IconEye :size="17" :stroke="2" />
			<span>{{ t('drive.preview') }}</span>
		</button>
		<button v-if="showStar" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-[#20262f]/70" @click="handleStar">
		...
		<button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-[#20262f]/70" :disabled="!canDownload" @click="handleDownload">
			<IconDownload :size="17" :stroke="2" />
			<span>{{ t('common.download') }}</span>
		</button>
		<button v-if="canRename" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-[#20262f]/70" :disabled="!canRename" @click="handleRename">
			<IconEdit :size="17" :stroke="2" />
			<span>{{ t('common.rename') }}</span>
		</button>
		<button v-if="canMove" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-[#20262f]/70" @click="handleMove">
			<IconArrowsMove :size="17" :stroke="2" />
			<span>Mover</span>
		</button>
		<button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-[#20262f]/70" :disabled="!canShowDetails" @click="handleDetails">
