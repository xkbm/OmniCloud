	...
	<div v-if="open" class="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 px-4 backdrop-blur-sm">
		<form class="w-full max-w-md rounded-[28px] border border-[#e0e3e7] bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.24)] dark:border-[#272e39] dark:bg-[#07090d]" @submit.prevent="submitForm">
			...
			<label class="mb-3 block text-sm">
				...
				<input v-model="form.email" type="email" required autocomplete="username" :placeholder="t('mega.emailPlaceholder')" class="h-11 w-full rounded-2xl border border-[#dadce0] bg-white px-4 outline-none placeholder:text-[#9aa0a6] focus:border-[#1a73e8] dark:border-[#272e39] dark:bg-[#12161d] dark:placeholder:text-slate-500" />
			</label>
			...
			<label class="block text-sm">
				...
				<input v-model="form.secondFactorCode" type="text" inputmode="numeric" autocomplete="one-time-code" placeholder="123456" class="h-11 w-full rounded-2xl border border-[#dadce0] bg-white px-4 outline-none placeholder:text-[#9aa0a6] focus:border-[#1a73e8] dark:border-[#272e39] dark:bg-[#12161d] dark:placeholder:text-slate-500" />
			</label>

			<div class="mt-6 flex items-center justify-end gap-3">
				<button type="button" class="h-10 rounded-full px-4 text-[#5f6368] hover:bg-[#f1f3f4] disabled:opacity-60 dark:text-slate-300 dark:hover:bg-[#1b2029]" :disabled="isConnecting" @click="closeModal">{{ t('common.cancel') }}</button>
